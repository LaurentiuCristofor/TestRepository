...
public class AirportDataProcessor
{
    ...
    protected static final String OUTPUT_FILENAME = "airport-one-edge.graphml";
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        output.write("\n");
        output.write("<!--airport data in graphml format (schema with one edge type)-->\n");
        output.write("\n");
        output.write("<!--Copyright (c) Laurentiu Cristofor-->\n");
        output.write("<!--This program is made available under the MIT License.-->\n");
        output.write("\n");
        ...
        output.write("  <key id='airline' for='node' attr.name='airline' attr.type='string'/>\n");
        output.write("  <key id='src_ap' for='node' attr.name='src_ap' attr.type='string'/>\n");
        output.write("  <key id='src_ap_id' for='node' attr.name='src_ap_id' attr.type='string'/>\n");
        output.write("  <key id='dst_ap' for='node' attr.name='dst_ap' attr.type='string'/>\n");
        output.write("  <key id='dst_ap_id' for='node' attr.name='dst_ap_id' attr.type='string'/>\n");
        ...
        output.write("\n");
        output.write("  <graph id='airport-one-edge' edgedefault='directed'>\n");
    ...
    protected static void processRoutes() throws IOException, ParseException
    {
        ...
        while ((columns = parser.getNextColumns()) != null)
        {
            ...
            // Output route edge.
            output.write(
                "  <edge id='" + flightNodeId
                + "' source='" + departureAirportNodeId + "' target = '" + arrivalAirportNodeId + "'>\n");
        
            output.write("    <data key='labelE'>route</data>\n");
            outputData(columns[RouteIndex.AIRLINE_IATA], "airline");
            outputData(columns[RouteIndex.AIRLINE_ID], "al_id");
            outputData(columns[RouteIndex.DEPARTURE_AIRPORT_IATA], "src_ap");
            outputData(columns[RouteIndex.DEPARTURE_AIRPORT_ID], "src_ap_id");
            outputData(columns[RouteIndex.ARRIVAL_AIRPORT_IATA], "dst_ap");
            outputData(columns[RouteIndex.ARRIVAL_AIRPORT_ID], "dst_ap_id");
            ...
            if (printDebuggingOutput)
            {
                System.out.println("route => " + flightNodeId);
                ...
                System.out.println("    airline " + airlineIata + "(" + airlineId + ")");
            }
